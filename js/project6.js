// Project 6: top nav hide/show on scroll (same behavior as Home/About)
const projectTopNav = document.querySelector('.project6-header.topbar');
...
// Project 6: shared More Works logic
if (typeof window.initProjectMoreWorks === 'function') {
  window.initProjectMoreWorks({
    currentProjectId: 'project6',
    currentProjectCategory: 'motion',
    classPrefix: 'project6'
  });
}

const watchVideoLink = document.querySelector('.project6-visit-btn[href="#final-video"]');
const finalVideoSection = document.getElementById('final-video');

if (watchVideoLink && finalVideoSection) {
...
// Project 6: initial load animation + hybrid scroll reveal animation
const project6Body = document.body;

if (project6Body && project6Body.classList.contains('project6-page')) {
  ...
  const initScrollReveal = () => {
    ...
    const tagVisualGroups = () => {
      const finalFigures = Array.from(document.querySelectorAll('.project6-final-grid figure'));
    ...
    const tagMoreWorksCards = () => {
      const cards = Array.from(document.querySelectorAll('#moreWorksGrid .project6-more-card'));
      cards.forEach((card, cardIndex) => {
        ...
        const safeInnerTargets = [
          card.querySelector('.project6-more-image'),
    ...
    project6Body.classList.add('project6-reveal-ready');
  ...
  const startProject4Animations = () => {
    prepareLoadElements();
    project6Body.classList.add('project6-load-running');

    if (prefersReducedMotion) {
      project6Body.classList.add('is-loaded');
      project6Body.classList.remove('project6-load-running');
      project6Body.classList.remove('project6-load-anim');
      initScrollReveal();
      return;
    }
