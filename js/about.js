...
// Bypass the Home intro only when navigating directly to Work or Contact.
(() => {
  const HOME_SECTION_HASHES = new Set(['#works', '#contact']);
  ...
  document.addEventListener('click', (event) => {
    ...
    const isHomeSection = pageName === 'index.html' && HOME_SECTION_HASHES.has(destination.hash);
    if (destination.origin !== window.location.origin || !isHomeSection) return;

    try {
      window.sessionStorage.setItem('portfolioIntroBypassOnce', 'true');
    } catch {
